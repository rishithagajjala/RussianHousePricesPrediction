...
# Making our Multiple Linear regression model without interaction terms
regressor = lm(formula = price_doc ~ ., data = training_set)
summary(regressor)

#Predicting test set results
y_pred = predict(regressor, newdata = test_set[-56])
...
regressor_int = lm(price_doc ~ full_sq +  life_sq + floor + max_floor + material + build_year + num_room + kitch_sq + state + product_type + 
